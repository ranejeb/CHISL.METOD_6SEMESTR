...
def fillTable(start, h, n, foo, fooExact, y0):
    ...
    for i in range(0, n):
        ...
        if i == 0:
            ...
            #Подсчет Yi по методу трапеций
            data[i][1] = data[i-1][1]+h/2*(foo(data[i-1][0], data[i-1][1])+foo(data[i][0], data[i-1][1]+h*foo(data[i-1][0], data[i-1][1])))
        #Точное решение
...
def trapezoidsTable(start, h, n, foo, fooExact, yo):
    #Подсчет таблицы
